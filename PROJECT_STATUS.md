...
사용자 요청으로 GitHub 저장소 생성을 시도했으나, 로컬에 **GitHub CLI(`gh`) 로그인이 안 되어 있어
대기 중**이다. `gh auth login`은 브라우저 인증이 필요한 대화형 명령이라 Claude가 대신 실행할 수
없다 — 사용자가 터미널(또는 `!gh auth login`)로 직접 로그인해야 한다. 로그인이 확인되면
`gh repo create`로 **private** 저장소를 만들고 최초 커밋/푸시를 진행하기로 합의된 상태다.
다음 세션에서 이 문서의 이 절이 그대로면 아직 로그인이 안 된 것이니, `gh auth status`로 먼저
확인할 것.
...
1. **GitHub 저장소 연결 마무리** — `gh auth login` 완료 후 이어서 진행.
2. **Node 서버(`server/`) 테스트 80개를 Java 쪽에도 확장 이관** — 지금 Java 쪽엔 핵심 시나리오 20개만 있음. 자녀 프로필, 아티클/광고 관리자 CRUD, 업로드 등은 아직 통합 테스트가 없음(수동으로는 검증했음).
3. **결제(PG) 연동** — 실제 PG사 계정/API 키 필요. 사용자 확인 후 진행.
4. **카테고리 관리 UI** — 지금은 DB에 직접 넣어야 함.
5. **관리자 목록 페이지네이션** — 상품/아티클이 많아지면 필요.
6. **소셜 로그인 앱 키 발급 및 적용** — 위 "소셜 로그인 자격증명 안내" 절 참고.
7. **`server/`(Node 버전) 처리 방향 결정** — 계속 참고용으로 남길지, 완전히 정리(삭제)할지 사용자 판단 필요. 삭제는 사용자가 명시적으로 요청할 때만 진행.
